...
class ReacherEnv(core.Env):
    ...
    def get_reward_old(self, prev_state, action, next_state):
        # TODO: Task 3: Implement and test two reward functions
        ########## Your code starts here ##########
        # A dummy reward, replace it with yours.
        prev_state_theta0 = prev_state[0]
        next_state_theta0 = next_state[0]
        
        diff = next_state_theta0 - prev_state_theta0
        return -(0.2-diff)**2
    ...
    def get_reward(self, prev_state, action, next_state):
        ...
        ########## Your code starts here ##########
        # A dummy reward, replace it with yours.
        #prev_state_cart = self.cartesian_pos(prev_state)
        next_state_cart = self.get_cartesian_pos(next_state)
        
        diff = np.linalg.norm(self.goal-next_state_cart)
        return -diff
