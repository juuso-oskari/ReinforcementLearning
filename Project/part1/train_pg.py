...
def train(agent, env):
    ...
    while not done:
        ...
        action, action_log_prob = agent.get_action(observation, evaluation=False)
        #previous_observation = observation.copy()

        # Perform the action on the environment, get new state and reward
        observation, reward, done, _ = env.step(to_numpy(action))

        # Store action's outcome (so that the agent can improve its policy)
        agent.record(action_log_prob, reward)
