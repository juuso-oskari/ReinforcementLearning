...
import scipy
import bsuite
sys.path.insert(1, '/u/44/korhonj19/unix/rl_course/')
...
class MCTS(object):
    ...
    def mcts(self, state) -> Node:
        ...
        for _ in range(self.num_simulations):
            ...
            while node.children:
                ...
                #act = self.puct(node)
                act = self.puct(node)
                # Or with greedy policy
                #act = self._argmax(node.children_values)
                
                new_node = node.children[act]
                
                node = new_node
                # reset env
                # self.env.reset()
                obs, reward, terminated, truncated = self.env.step(act)
                node.reward = reward
                node.terminal = terminated
            ...
            while trajectory:
                ...
                #v0 = R0 + y*R1 + y^2*R2 + y^3*R3
                # 2.update node (total_value, visit_count)
                node.total_value += ret # same as averaging after simulations
                node.visit_count += 1
    ...
    def puct(self, node: Node, ucb_scaling: float = 1.):
        ...
        # change these values!
        value_scores = node.children_values #np.zeros(self.num_actions)
        priors = np.array([child.prior for child in node.children.values()]) # np.zeros(self.num_actions)
        
        Nsb = np.sum(node.children_visits) # visit counts of all children nodes summed
        Nsa = node.children_visits
        c1 = 1.25
        c2 = 19652
        visit_ratios =  np.sqrt(Nsb)/ (1+Nsa) #*(c1 + np.log( (Nsb+c2+1) / c2 ))# np.zeros(self.num_actions)
        # Combine.
...
class AZAgent(object): 
    ...
    def update(self, data): 
        ...
        # See buffer
        # buffer add: buffer.add(state, action, next_state, reward, done, {'pi': probs})
        
        target_probs = data.extra['pi'] # torch.stack(self.action_probs, dim=0).to(device).squeeze(-1)
        rewards = data.reward  #torch.stack(self.rewards, dim=0).to(device).squeeze(-1)
        states = data.state # torch.stack(self.states, dim=0).to(device).squeeze(-1)
        next_states = data.next_state #torch.stack(self.next_states, dim=0).to(device).squeeze(-1)
        dones = 1 - data.not_done # torch.stack(self.dones, dim=0).to(device).squeeze(-1)
        
        # 1. calculate the TD target as well as the MSE loss between the predicted value and the TD target
        with torch.no_grad():
           TD_targ = rewards + self.gamma * self.value.forward(next_states)*(1-dones)  #self.value.forward(next_states)*(1-dones)
        
        TD_pred = self.value.forward(states)
        loss1 = F.mse_loss(TD_pred, TD_targ.detach())
        ...
        # Possibly detach 
        with torch.no_grad():
            Advantage = rewards + (self.gamma * self.value.forward(states)) - self.value.forward(next_states)*(1-dones)
            Advantage = (Advantage - torch.mean(Advantage)) / torch.std(Advantage)
        
        # I don't know how to use Advantage
        #Advantage = Advantage.detach()
        #loss = -torch.mean(act_pr * Advantage.detach())
        #ce_loss= nn.CrossEntropyLoss()
        #predicted_probs = torch.squeeze(self.value(states))
        #loss = ce_loss(predicted_probs, torch.argmax(target_probs, dim = 1))
        loss = self.criterion(self.policy.forward(states), data.extra['pi']).mean()
