    ...
    def plan(self, obs, t0, eval_mode=False):
        ...
        with Parallel(n_jobs=-1,) as parallel:  # we use joblib.Parallel to parallel the evaluation.
            ...
            for _ in range(self.iteration):
                ...
                #    #print(parallel(delayed(rollout_simulator))(model, action))
                #    value = delayed(rollout_simulator)(model, action)
                #    values.append(value)
                #    actions.append(action)
                actions = [np.random.normal(mean, std) for sample in range(self.num_samples)]
                #actions = np.swapaxes(actions, 0, 1)
                #print(self.num_samples)
                #print(np.shape(actions))
                values = parallel(delayed(rollout_simulator)(model, action) for action in actions)
                #print(np.shape(values))
                values = np.asarray(values)
                actions = np.asarray(actions)
                top_idx = np.argsort(values, )[::-1][:self.num_topk] # select top samples
                _mean, _std = np.mean(actions[top_idx], axis=0), np.std(actions[top_idx], axis=0) # change this line 
