...
def get_action(state, q_axis, q_table, epsilon=0.0):
    ...
    if u < epsilon:
        a = np.random.randint(low=0, high=len(q_table[idx])) # random action from the action space
    else:
        a = np.argmax(q_table[idx])
...
def update_q_value(old_state, action, new_state, gamma, reward, done, alpha, q_axis, q_table):
    ...
    ########## Your code starts here ##########
    max_a = np.max(q_table[new_table_idx])
    if done:
        max_a = 0
    q_table[old_table_idx][action] = q_table[old_table_idx][action] + alpha * (reward + gamma * max_a - q_table[old_table_idx][action])
    ########### Your code ends here ##########
    return q_table
