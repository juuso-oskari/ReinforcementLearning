...
class DQNAgent(object):
    ...
    def update(self, buffer):
        ...
        # calculate the q(s,a)
        qs=self.policy_net(torch.tensor(batch.state)).gather(1, batch.action.type(torch.LongTensor))
        # calculate q target
        R = torch.tensor(batch.reward).squeeze().to(device)
        g = torch.tensor(self.gamma).squeeze().to(device)
        status = torch.tensor(batch.not_done).squeeze().to(device)
        qs_next = self.target_net(torch.tensor(batch.next_state).squeeze()).max(1)[0].detach()
        q_tar = R + g * status * qs_next
        # for loop implementation (reeeaaaallly sloow)
        """
        q_tar2 = torch.zeros_like(qs)
        for i in range(qs.size(0)):
            reward = batch.reward[i]
            status = batch.not_done[i]
            if status==0:
                q_tar2[i] = reward
            else:
                q_next_state = torch.max(self.target_net(batch.next_state[i,:]))
                q_tar2[i] = reward + self.gamma * q_next_state
        """
        #calculate the loss 
        #mse_loss=nn.MSELoss()
        self.optimizer.zero_grad()
        loss = F.smooth_l1_loss(qs.squeeze(), q_tar)
        loss.backward()
    ...
    def get_action(self, state, epsilon=0.05):
        ...
        if u < epsilon:
            ...
            return a #torch.randint(low=0, high=self.n_actions, size=(1,1))
        else:
            q_values = self.policy_net(torch.tensor(state).to(device))
            amax = torch.Tensor.cpu(torch.argmax(q_values).detach())
            #print(amax)
            return amax.numpy()
